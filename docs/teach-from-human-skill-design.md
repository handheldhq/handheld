...
1. **MCP tools: `teach_request`, `teach_status`, and `read_teach_artifact`**. `teach_request` has signature roughly `{ objective: string, package?: string }`; it opens the headed viewer, starts the recorder, and returns immediately with a `teachId`. Locked MCP-only agents then poll `teach_status` until the envelope is `ready`, and call `read_teach_artifact` to read the captured trajectory JSON without host filesystem access. Because these are handheld MCP tools, every `run` workspace (which is locked to `mcp__handheld__*`) exposes them automatically. **The agent discovers them the same way it discovers `tap`.**
...
5. **Handover gate UX.** Handing off to a human is novel for this CLI. If the human walks away, the teach envelope must eventually move to `timeout` and the agent must degrade gracefully (report "human did not complete demonstration", fall back to asking a question). The MCP polling contract needs careful timeout + cancellation handling.
...
- **Phase 1 — MVP recorder + synthesis → markdown.** `handheld teach` opens the headed viewer (reuse `connectDevice`), runs the state-sampled poll-loop recorder, writes `trajectory.json`, and the `teach-from-human` skill synthesizes a **domain-skill markdown** (finding A methodology). Replay is LLM-interpreted (the agent reads the markdown). MCP `teach_request` + `teach_status` + `read_teach_artifact` provide the locked-agent handover. **No deterministic replay yet** — markdown + agent is the whole loop. Ship the security constraint (no stored credentials) from day one.
